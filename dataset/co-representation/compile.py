...
if __name__ == '__main__':
    # paths
    path_root = './'
    path_indir = os.path.join( path_root, 'words/ailabs')
    ...
    # split train/test
    validation_songs = json.load(open('../../dataset_wayne/representations/validation_songs.json', 'r'))
    train_idx = []
    test_idx = []
    ...
    # save validation map 
    path_fn2idx_map = os.path.join(path_root, 'ailabs_valid_fn2idx_map.json')
    with open(path_fn2idx_map, 'w') as f:
        json.dump(fn2idx_map, f)

    # save train map
    path_train_fn2idx_map = os.path.join(path_root, 'ailabs_train_fn2idx_map.json')
    with open(path_train_fn2idx_map, 'w') as f:
        json.dump(train_fn2idx_map, f)


    # save train
    path_train = os.path.join(path_root, 'ailabs_train_data_{}'.format(COMPILE_TARGET))
    path_train += '.npz'
    np.savez(
        path_train, 
        x=x_final[train_idx],
    ...
    # save test
    path_test = os.path.join(path_root, 'ailabs_test_data_{}'.format(COMPILE_TARGET))
    path_test += '.npz'
